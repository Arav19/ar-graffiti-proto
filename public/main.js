...
/* ================ Prevent double-run from injected content-scripts ================ */
// If an extension or hot-reload tries to run the module twice, short-circuit:
if (window.__surfaceless_main_loaded) {
  console.warn("main.js already loaded in this page — skipping re-execution");
} else {
  ...
  window.addEventListener("DOMContentLoaded", () => {
    ...
    // Basic sanity:
    if (!canvasEl) {
      console.error("three-canvas missing from HTML");
      if (statusEl) statusEl.textContent = "UI error: canvas missing";
      return;
    }
    if (!startBtn || !stopBtn || !clearBtn) {
      console.error("required UI buttons missing");
      if (statusEl) statusEl.textContent = "UI error: missing buttons";
      return;
    }
    ...
    /* ====== helpers: reticle, circle, plane mesh ====== */
    ...
    onChildAdded(planesRef, (snap) => {
      ...
      // If meta has pos, place accordingly; otherwise place at reticle or fallback.
    ...
    if (placeCanvasBtn) {
      placeCanvasBtn.addEventListener("click", async () => {
        ...
        try {
          // attempt to sample geolocation for better geo anchoring (best-effort)
          const avg = await (async function sampleAndAverageGPS(n = 5, delayMs = 300) {
            const samples = [];
    ...
    /* ================= Init status ================= */
    if (statusEl) statusEl.textContent = "Ready — enable camera, place canvas, then Start Drawing";
